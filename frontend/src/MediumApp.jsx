...
export default function MediumApp({ onSwitchToFull }) {
  ...
  const USAGE_TIMEOUT_MS = 10 * 60 * 1000; // 10 min — covers a recovering 429 backoff (see App.jsx)
  ...
  // (used % vs how far through the 7 days you are). Same look as the Session burn; ETA only when a real threat.
  const weeklyBurnIndicator = (() => {
    const sd = claudeUsage?.seven_day;
          ...
          <span className="relative group cursor-default">
            ...
            <span className={`absolute bottom-full right-0 mb-1.5 hidden group-hover:flex flex-col gap-1 px-2.5 py-2 rounded-lg shadow-xl border ${colors.border} ${colors.bg.header} text-[10px] whitespace-nowrap z-50`}>
              ...
              {(tokens.monthModelUsage?.Fable?.estimatedCost || 0) > 0 && (
                <span className="flex items-center gap-1.5"><span className={colors.model?.fable?.text || 'text-amber-400'}>✦</span><span className={colors.text.muted}>Fable</span><span className={`font-mono ml-auto ${colors.model?.fable?.text || 'text-amber-400'}`}>${(tokens.monthModelUsage?.Fable?.estimatedCost || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span></span>
              )}
